...
- [ ] T097 CRITICAL: Generate runner-up differentiation text in src/engine/recommendationEngine.ts from the runner-up's missing signals and matched red flags in rules.json instead of the generic template string, so every runner-up row cites the framework per Constitution I & III, FR-007, SC-002 (contradicts)
- [ ] T098 CRITICAL: Populate `tiebreakers[]` in src/data/rules.json with at least one tiebreaker question (including a question entry with `isTiebreaker: true`, position 6+, and questionMappings) per FR-005a, SC-007 (missing)
- [ ] T099 CRITICAL: Wire tie handling into the wizard in src/App.tsx — after the last core question call `detectTie()`/`applyTiebreakerSignals()` and present the tiebreaker question before showing results per FR-005, SC-007 (missing)
- [ ] T100 Create tests/integration/wizard-flow.test.tsx covering Q1→results navigation, Next/Back buttons, and answer persistence per US1/AC1-AC4 (missing)
- [ ] T101 Create tests/integration/recommendation.test.tsx and tests/integration/comparisonTable.test.tsx verifying justification cites framework signals and 1–2 runner-up rows render with differentiators per US2/AC2, US3/AC2 (missing)
- [ ] T102 Add playwright.config.ts and tests/e2e/wizard.spec.ts so `npm run test:e2e` runs the full wizard flow per plan: testing strategy (missing)
- [ ] T103 Create tests/e2e/offline.spec.ts and tests/unit/offline.test.ts asserting zero network requests and no localStorage/sessionStorage writes during the full flow per FR-008, FR-011, SC-004, SC-005, US4/AC1-AC4 (missing)
- [ ] T104 Create tests/e2e/accessibility.spec.ts using axe-core plus keyboard-only navigation coverage per DR-001, SC-006 (missing)
- [ ] T105 Create tests/unit/rules.test.ts validating rules.json schema: 5 tools, signal/red-flag weights within 1-10, 5-7 core questions, every question has questionMappings per FR-001, FR-013, SC-008 (missing)
- [ ] T106 Update src/components/ProgressIndicator.tsx to render "Question X of Y" and expose role="progressbar" with aria-valuenow/aria-valuemin/aria-valuemax and an aria-live announcement per FR-003, DR-001, DR-004 (partial)
- [ ] T107 Extract wizard state from src/App.tsx into src/hooks/useWizardState.ts and src/components/WizardContainer.tsx, and move helper logic to src/utils/scoring.ts and src/utils/formatting.ts per plan: Project Structure (partial)
- [ ] T108 Add an error boundary around the wizard in src/App.tsx and call `loadRulesFile()` at startup so malformed rules.json shows "Unable to load framework data, please refresh" per plan: error handling, FR-013 (missing)
- [ ] T109 Add responsive styling with breakpoints at 320px / 768px / 1920px, including stacked comparison-table rows on mobile per DR-002, DR-006 (partial)
- [ ] T110 Audit src/data/rules.json against docs/decision-framework.md and add any missing signals, red flags, and tool mappings so recommendations are grounded exclusively in the framework per FR-004, FR-012, Constitution III (partial)
- [ ] T111 Add .eslintrc.json and .prettierrc so `npm run lint` and `npm run format` execute successfully per plan: tooling (missing)
- [ ] T112 Add vite.config.ts with the React plugin, build target, and deployment base path per plan: Vite build configuration (missing)
- [ ] T113 Add .github/workflows CI running type-check, unit, integration, and e2e tests plus production build on push/PR per plan: production readiness (missing)
- [ ] T114 Add CONTRIBUTING.md and src/CONTRIBUTING-ENGINE.md documenting setup, test commands, and how to extend tools/signals/questions per plan: documentation (missing)
- [ ] T115 Execute quickstart.md scenarios 1–4, 7, and 8 against the running app and record outcomes per SC-001, SC-007 (missing)
