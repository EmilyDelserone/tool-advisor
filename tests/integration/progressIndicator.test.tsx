...
describe('ProgressIndicator (FR-003, DR-004)', () => {
  ...
  it('reports percentage complete', () => {
    render(<ProgressIndicator currentIndex={3} totalQuestions={8} />);

    expect(screen.getByText('50% complete')).toBeTruthy();
  });

  it('exposes progressbar semantics', () => {
    render(<ProgressIndicator currentIndex={2} totalQuestions={7} />);

    const bar = screen.getByRole('progressbar');
    expect(bar.getAttribute('aria-valuenow')).toBe('3');
    expect(bar.getAttribute('aria-valuemin')).toBe('1');
    expect(bar.getAttribute('aria-valuemax')).toBe('7');
    expect(bar.getAttribute('aria-valuetext')).toBe('Question 3 of 7');
  });
  ...
  it('reaches 100% on the final question', () => {
    render(<ProgressIndicator currentIndex={6} totalQuestions={7} />);
